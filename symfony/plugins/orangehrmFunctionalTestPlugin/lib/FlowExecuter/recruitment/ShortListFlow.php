...
/**
 * Description of shortListFlow
 *
 * @author Faris
 */
class ShortListFlow extends flow {

    public $dataArray;
    public $selenium;
    public $shortListPageObject;
    ...
    public function execute($verify=true) {
        ...
        $this->shortListPageObject = $editCandidate->editStatus($this->dataArray['status']); // edit as shortlist for the candidate specified above
        $this->shortListPageObject->shortlistTheCandidate($this->dataArray['notes']);
        if ($verify) {
            return $this->verify();
        }
        return true;
    ...
    public function verify() {
        ...
        return RecruitmentHelper::isCandidateStatus($this->selenium, $candidateFullName, "Shortlisted");
    }
