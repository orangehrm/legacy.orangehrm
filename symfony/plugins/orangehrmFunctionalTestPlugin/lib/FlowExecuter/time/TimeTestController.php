...
/**
 * Description of TimeTestController
 *
 * @author Faris
 */
class TimeTestController extends TestController {

    private $flowData;
    private $timePrerequisites;
    public $sharedData;
    ...
    public function __construct($selenium, $fixture, $section) {
        ...
        $this->timePrerequisites = new TimePrerequisiteHandler($this->flowData["PrerequisiteDetails"]["fileName"]);
        $this->flowData = $this->flowData[$section];

        //$this->substituteWithPrerequisiteDetails();

        $flowMapper = new TimeFlowMapper($selenium);
    ...
    private function getPrerequisiteDetails($prerequisiteIndex) {
        ...
        if ($exploded[1] == "LeaveRequest") {
            ...
            return $this->timePrerequisites->getPrerequisiteDetailsIntoAMergedArray($prerequisite);
        }
    }
