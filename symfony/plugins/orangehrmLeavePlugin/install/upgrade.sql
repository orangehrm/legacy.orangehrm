...
alter table `hs_hr_leave_type` add column int_id int not null auto_increment unique key;

INSERT INTO `ohrm_leave_type` (`id`, `name`, `deleted`, `operational_country_id`)
                    SELECT hs_hr_leavetype.`int_id`, hs_hr_leavetype.`leave_type_name`, 
                    IF(hs_hr_leavetype.`available_flag` = 1, 0, 1 , hs_hr_leavetype.`operational_country_id`)
                    FROM `hs_hr_leavetype`;
