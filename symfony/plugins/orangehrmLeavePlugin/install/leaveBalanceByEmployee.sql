...
<filter_fields>
	<input_field type="text" name="employeeName" label="Employee Name"></input_field>
...
<sub_report type="sql" name="subTable1">
                    <query>
FROM ohrm_leave_entitlement where emp_number = 1 
...
<sub_report type="sql" name="subTable2">
                    <query>
FROM ohrm_leave_entitlement WHERE emp_number = 1 AND
from_date &lt; curdate()
GROUP BY leave_type_id
ORDER BY ohrm_leave_entitlement.leave_type_id
</query>
...
<query>
FROM ohrm_leave WHERE employee_id = 1 AND
leave_status = 3
GROUP BY leave_type_id
