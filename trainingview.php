...
$baseURL = './CentralController.php?trainingcode='. $this->getArr['trainingcode'];

$allowAdd = $locRights['add'];
$allowDelete = $locRights['delete'];
$showSearch = isset($this->popArr['showSearch']) ? $this->popArr['showSearch']: true;

if (isset($this->popArr['columnsToSkip'])) {
	$columnsToSkip = $this->popArr['columnsToSkip'];
} else {
	$columnsToSkip = 0;
}
          ...
          <td class="tableTopRight"></td>
    		...
    		<tr>
       		...
       		<td class="<?php echo $cssClass?>" width="50"></td>
		 		<?php 	}  ?>
		 		<td class="<?php echo $cssClass?>" width="250"><a href="<?php echo $baseURL . '&id='. $list[$j][0];?>&action=View" class="listViewTdLinkS1"><?php echo $list[$j][$columnsToSkip]?></a></td>
		 		<?php
		 			  $k=1;
		 			  if ($k < count($headings)) {
